...
for i_sub = 9:size(subses,1)
    ...
    if exist(fullfile(save_path, [params.sub '_opmeeg_timelocked.mat']),'file') && overwrite==false
        ...
        params.layout = 'fieldlinebeta2bz_helmet.mat';
        params.chs = '*bz';
        opm_timelocked = timelock_MEG(opm_cleaned, save_path, params);
        close all

        params.modality = 'opmeeg';
        params.layout = opmeeg_layout;
        params.chs = 'EEG*';
        opmeeg_timelocked = timelock_MEG(opmeeg_cleaned, save_path, params);
        close all
    end
end
...
for i_sub = 1:size(subses,1)
    params.sub = ['sub_' num2str(i_sub)];

    %% Paths
    raw_path = fullfile('/archive/21099_opm/MEG/',['NatMEG_' subses{i_sub,1}], subses{i_sub,2});
    save_path = fullfile('/home/chrpfe/Documents/21099_opm/Phalanges',params_sub);
    meg_path = fullfile('/Users/christophpfeiffer/data_archive/21099_opm/MEG/',['NatMEG_' subsub{i_sub,1}], subses{i_sub,2});
    mri_path = fullfile('/Users/christophpfeiffer/data_archive/21099_opm/MRI/',['NatMEG_' subsub{i_sub,1}]);
    if ~exist(save_path, 'dir')
       mkdir(save_path)
    end
    if ~exist(fullfile(save_path,'figs'), 'dir')
    ...
    opm_file = fullfile(raw_path, 'osmeg', 'PhalangesOPM_raw.fif');
    aux_file = fullfile(raw_path, 'meg', 'PhalangesEEG.fif');
    hpi_file = fullfile(meg_path, 'osmeg', 'HPIpre_raw.fif');

    %% Preprocess cvMEG
    % Read data 
    if exist(fullfile(save_path, 'meg_data.mat'),'file') && overwrite==false
        load(fullfile(save_path, 'meg_data.mat'));
    else
        [meg_data_filt, trl_meg] = read_cvMEG(meg_file, save_path, params);
    end
    %% Filter and reject artifacts
    if exist(fullfile(save_path, 'meg_cleaned.mat'),'file') && overwrite==false
        load(fullfile(save_path, 'meg_cleaned.mat'));
    else
        meg_cleaned = preprocess_cvMEG(meg_data_filt, save_path);
    end
    %% Remove eye and heart artifacts with ICA
    if exist(fullfile(save_path, 'meg_ica.mat'),'file') && overwrite==false
        load(fullfile(save_path, 'meg_ica.mat'));
    else
        meg_ica = ica_cvMEG(meg_cleaned, save_path, params);
    end
    %% Average
    if exist(fullfile(save_path, 'meg_timelocked.mat'),'file') && overwrite==false
        load(fullfile(save_path, 'meg_timelocked.mat'));
    else
        for i_phalange = 1:5
            cfg = [];
            cfg.covariance          = 'yes';
            cfg.covariancewindow    = 'prestim';
            cfg.trials = find(meg_ica.trialinfo==params.trigger_code(i_phalange));
            meg_timelocked{i_phalange} = ft_timelockanalysis(cfg, meg_ica);
        end
        save(fullfile(save_path, 'meg_timelocked'), 'meg_timelocked', '-v7.3'); disp('done');
    end
...
for i_sub = 1:size(subses,1)
    %% Evoked figures
    if ~exist(fullfile(save_path, 'figs', ['opm_evoked_multiplot_sub-' num2str(i_sub,'%02d') '.fig']),'file') || overwrite==true    
        plot_sub_evoked(save_path, i_sub, params.phalange_labels, opm_timelocked, meg_timelocked);
    end
    close all
    ...
    if exist(fullfile(save_path, 'opm_trans.mat'),'file') && overwrite==false
        ...
        hpi_file = fullfile(meg_path, 'osmeg', 'HPI_raw.fif');
        aux_file = fullfile(meg_path, 'meg', 'PhalangesEEG.fif');
...
bar(1:5,mean(peak_ratio,1));
hold on
er = errorbar(1:5,mean(peak_ratio,1), mean(peak_ratio,1)-min(peak_ratio,[],1), mean(peak_ratio,1)-max(peak_ratio,[],1));    
er.Color = [0 0 0];                            
er.LineStyle = 'none';  
er.LineWidth = 1;
er.CapSize = 30;
hold off
title(['M100 peak amplitude ratio (mean = ' num2str(mean(mean(peak_ratio))) ')'])
ylabel('SQUID/OPM')
xlabel('Phalange')
xticklabels(phalange_labels)
savefig(h, fullfile('/Users/christophpfeiffer/data_local/Benchmarking_phalanges/', 'PeakAmpRatio_M100.fig'))
