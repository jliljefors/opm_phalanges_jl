function opm_cleaned = read_osMEG(opm_file, aux_file, save_path, params,trialinfo)
...
trig = opm_raw.trial{1}(opm_trig,:) == 1;
...
aux_trig = find(contains(aux_raw.label,'STI001'));
...
trl_aux(:,4) = aux_raw.trial{1}(aux_trig,trig)/5;
...
TRIG_channels = find(contains(aux_epo.label,'STI'));   % CHANGE FROM STI101
...
if ~exist(fullfile(save_path, 'figs', [params.sub, '_opm_spectrum0.jpg']),'dir')
    mkdir(fullfile(save_path, 'figs', params.sub))
end
